...
#ifndef __EightQueens__BFS__
#define __EightQueens__BFS__
...
public:
    ...
    void runBFS(T domain, S initialState, R goalState){ // Runs the BFS algorithm
        ...
        while(!(domain.goalState(queue.front().state, goalState))){  // Loops to add nodes to queue
            ...
            tempList = domain.successorFn(&queue.front(), &rootNode);   // Takes in list of children
...
#endif /* defined(__EightQueens__BFS__) */
